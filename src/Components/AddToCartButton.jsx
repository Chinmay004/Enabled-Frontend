...
//version 2 dropping this make it more optimzable
...
//             <button
//               onClick={handleGoToCart}
//               className="bg-green-600 border border-green-600 hover:bg-green-700 text-white py-1 px-3 rounded transitions w-full "
//             >
//               Go to Cart
//             </button>
//           )}
  ...
  useEffect(() => {
    const checkCartStatus = async () => {
      ...
      try {
        ...
        const [idToken, exists, result] = await Promise.all([
          idTokenPromise,
          checkProductInCart(await idTokenPromise, productId),
          getUserProductPurchaseCount(await idTokenPromise, productId),
        ]);
  ...
  const handleAddToCart = async () => {
    try {
      ...
      const [idToken, result] = await Promise.all([
        idTokenPromise,
        getUserProductPurchaseCount(await idTokenPromise, productId, userId),
      ]);
      ...
      if (totalPurchased + quantity > maxLimit) {
        alert(
          `You can only purchase ${maxLimit} units of this product in a year. You've already bought ${totalPurchased}.`
        );
        setAdded(false);
        return;
      }
    ...
    <div className="flex gap-10 w-full font-inter">
      {!added ? (
        <>
          ...
          {showCounter && (
            <div className="flex items-center gap-2 justify-center border rounded border-[#e5e7eb]">
              <button onClick={decrement} className="px-2 py-1 bg-gray-200 text-lg">
                -
              </button>
              <span className="font-semibold">{quantity}</span>
              <button onClick={increment} className="px-2 py-1 bg-gray-200 text-lg">
                +
              </button>
            </div>
          )}
